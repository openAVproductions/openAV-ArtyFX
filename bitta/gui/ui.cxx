...
LV2UI_Handle bitta_instantiate(const struct _LV2UI_Descriptor * descriptor,
...
{
  if (strcmp(plugin_uri, BITTA_URI) != 0) {
      fprintf(stderr, "BITTA_URI error: this GUI does not support plugin with URI %s\n", plugin_uri);
      return NULL;
  }
...
void bitta_cleanup(LV2UI_Handle ui) {
  GUI *pluginGui = (GUI *) ui;
  delete pluginGui->widget;
  free( pluginGui);
}

void bitta_port_event(LV2UI_Handle ui,
               uint32_t port_index,
...
int bitta_idle(LV2UI_Handle handle)
{
  GUI* self = (GUI*)handle;
...
const LV2UI_Idle_Interface idle_iface = { bitta_idle };

const void* bitta_extension_data(const char* uri)
{
