...
LV2UI_Handle satma_instantiate(const struct _LV2UI_Descriptor * descriptor,
...
{
  if (strcmp(plugin_uri, SATMA_URI) != 0) {
      fprintf(stderr, "SATMA_URI error: this GUI does not support plugin with URI %s\n", plugin_uri);
      return NULL;
  }
...
void satma_cleanup(LV2UI_Handle ui) {
  GUI *pluginGui = (GUI *) ui;
  delete pluginGui->widget;
  free( pluginGui);
}

void satma_port_event(LV2UI_Handle ui,
               uint32_t port_index,
               ...
               const void * buffer)
{
  ...
  if ( format == 0 )
  {
    ...
    switch ( port_index )
    {
      ...
      case SATMA_TONE:
          {
            self->widget->graph->setEnvelope( value );
          }
      case SATMA_ACTIVE:
          {
...
int satma_idle(LV2UI_Handle handle)
{
  GUI* self = (GUI*)handle;
...
static const LV2UI_Idle_Interface idle_iface = { satma_idle };

const void* satma_extension_data(const char* uri)
{
