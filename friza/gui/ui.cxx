...
{
  if (strcmp(plugin_uri, FRIZA_URI) != 0) {
      fprintf(stderr, "FRIZA_URI error: this GUI does not support plugin with URI %s\n", plugin_uri);
      return NULL;
  }
...
void friza_cleanup(LV2UI_Handle ui) {
  GUI *pluginGui = (GUI *) ui;
  delete pluginGui->widget;
  free( pluginGui);
}

void friza_port_event(LV2UI_Handle ui,
               uint32_t port_index,
...
int friza_idle(LV2UI_Handle handle)
{
  GUI* self = (GUI*)handle;
...
const LV2UI_Idle_Interface idle_iface = { friza_idle };

const void* friza_extension_data(const char* uri)
{
