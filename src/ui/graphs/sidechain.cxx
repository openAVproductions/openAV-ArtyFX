...
Sidechain::Sidechain( Avtk::UI* ui, int x_, int y_, int w_, int h_, std::string label_) :
...
void Sidechain::draw( cairo_t* cr )
{
  ...
  */
  ...
  // imported from NTK sidechain widget
  {
    ...
    cairo_move_to( cr, x + w * 0.750 - (w * 0.5 * (1-threshold)), y  );
    
    cairo_line_to( cr, x + w * 0.750 - (w * 0.5 * (1-threshold)) + value()* reduceAmount*( w * 0.5 ), y + h / 2 );
