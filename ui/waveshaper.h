  ...
  public:
    ...
    void draw()
    {
      if (damage() & FL_DAMAGE_ALL)
      {
        ...
        // Tone bars
        cairo_move_to( cr, x + w/4, y + 2 );
        cairo_line_to( cr, x + w/4, y + h / 2 * envelope );
        
        cairo_move_to( cr, x + w - w/4, y + h - 2 );
        cairo_line_to( cr, x + w - w/4, y + h - (h / 2 * envelope) );
        
        //cairo_set_source_rgba (cr, 0.0, 0.48, 1.0, 0.6 );
        cairo_set_source_rgba (cr, 0,0,0, 0.7 );
        cairo_set_line_width(cr, 21.5);
