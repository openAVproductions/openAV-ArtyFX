...
LV2UI_Handle driva_instantiate(const struct _LV2UI_Descriptor * descriptor,
...
{
  if (strcmp(plugin_uri, DRIVA_URI) != 0) {
      fprintf(stderr, "DRIVA_URI error: this GUI does not support plugin with URI %s\n", plugin_uri);
      return NULL;
  }
...
void driva_cleanup(LV2UI_Handle ui) {
  GUI *pluginGui = (GUI *) ui;
  delete pluginGui->widget;
  free( pluginGui);
}

void driva_port_event(LV2UI_Handle ui,
               uint32_t port_index,
...
int driva_idle(LV2UI_Handle handle)
{
  GUI* self = (GUI*)handle;
...
const LV2UI_Idle_Interface idle_iface = { driva_idle };

const void* driva_extension_data(const char* uri)
{
