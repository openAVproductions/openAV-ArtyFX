...
LV2UI_Handle capta_instantiate(const struct _LV2UI_Descriptor * descriptor,
...
{
  if (strcmp(plugin_uri, CAPTA_URI) != 0) {
      fprintf(stderr, "CAPTA_URI error: this GUI does not support plugin with URI %s\n", plugin_uri);
      return NULL;
  }
...
void capta_cleanup(LV2UI_Handle ui) {
  GUI *pluginGui = (GUI *) ui;
  delete pluginGui->widget;
  free( pluginGui);
}

void capta_port_event(LV2UI_Handle ui,
               uint32_t port_index,
...
int capta_idle(LV2UI_Handle handle)
{
  GUI* self = (GUI*)handle;
...
const LV2UI_Idle_Interface idle_iface = { capta_idle };

const void* capta_extension_data(const char* uri)
{
