...
LV2UI_Handle della_instantiate(const struct _LV2UI_Descriptor * descriptor,
...
{
  if (strcmp(plugin_uri, DELLA_URI) != 0) {
      fprintf(stderr, "DELLA_URI error: this GUI does not support plugin with URI %s\n", plugin_uri);
      return NULL;
  }
...
void della_cleanup(LV2UI_Handle ui) {
  GUI *pluginGui = (GUI *) ui;
  delete pluginGui->widget;
  free( pluginGui);
}

void della_port_event(LV2UI_Handle ui,
               uint32_t port_index,
...
int della_idle(LV2UI_Handle handle)
{
  GUI* self = (GUI*)handle;
...
static const LV2UI_Idle_Interface idle_iface = { della_idle };

const void* della_extension_data(const char* uri)
{
